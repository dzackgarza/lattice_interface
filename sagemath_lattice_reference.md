...
| Method | Description | Tags |
|--------|-------------|------|
| `orthogonal_group(gens, is_finite)` | Matrix group of isometries of ambient space preserving L; generators computed automatically if `gens=None` | `[PD only — indefinite not yet implemented]` |
| `automorphisms(gens, is_finite)` | Alias for `orthogonal_group()` | `[PD only]` |
...
| `is_genus(sig, even=True)` | Whether there exists a lattice with this discriminant form and signature `sig`; set `even=False` to test odd lattices (odd case partially implemented) | |
| `genus(sig)` | `Genus` object compatible with this discriminant form and signature `sig`; raises `ValueError` if none exists | |
...
- **`orthogonal_group()` / `automorphisms()` on `IntegralLattice`** requires positive definite — indefinite case not yet implemented.
...
- **`TorsionQuadraticModule.is_genus(sig, even=True)`** — the `even` parameter (default `True`) determines whether odd lattices are considered. For odd lattices set `even=False`, but note the odd case is only partially implemented in Sage 10.8.
...
- **`IntegralLattice.orthogonal_group()` requires positive definite; `GroupOfIsometries` does not.** The former is a high-level lattice method that computes Aut(L) automatically (needs `sage.libs.gap` and the lattice to be PD). The lower-level `GroupOfIsometries(degree, ZZ, gens, bil)` accepts any generators and any symmetric bilinear form, including indefinite ones — but you must supply the generators yourself; it does not compute them.
