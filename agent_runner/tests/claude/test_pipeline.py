...
def test_claude_pipeline(recwarn):
    rc = Orchestrator(args=OrchestratorArgs(agent="claude", task="debug_hello_simple")).run()
    if rc == 10:
        assert any(issubclass(w.category, RuntimeWarning) for w in recwarn.list)
        return
    assert rc == 0
    meta = _load_metadata("debug_hello_simple", "claude")
