...
| Method | Description | Tags |
|--------|-------------|------|
| `orthogonal_group(gens=None, is_finite=None)` | Orthogonal group $O(L)$ as a `GroupOfIsometries` (matrix group preserving the bilinear form). If `gens` is given, returns the subgroup generated by `gens`. `is_finite` can be set to `True`/`False` to hint whether the group is finite | `[DEFINITE]` |
