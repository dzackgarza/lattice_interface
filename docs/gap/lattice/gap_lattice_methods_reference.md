...
| `NormalFormIntMat(mat, options)` | General integer normal-form computation; `options` is an integer bitmask: bit 0 selects SNF (1) vs. triangular/HNF (0); bit 1 reduces off-diagonal; bit 2 includes row transforms; bit 3 includes col transforms; bit 4 (value 16) makes the operation destructive. Returns a record with `normal` (the computed form), optionally `rowtrans` (n×n unimodular) and/or `coltrans` (m×m unimodular) transform matrices, `signdet` (sign of determinant), and `rank`. | `[CORE, ZZMOD]` |
...
| `Decomposition(A, B, depth)` | For an m×n cyclotomic matrix `A` of rank m ≤ n and a list `B` of cyclotomic vectors of length n, finds integral solutions x of `x * A = B[i]` via p-adic series. `depth` is a nonnegative integer (number of p-adic iterations, prime p=83 by default) or the string `"nonnegative"` (assumes nonneg integer solutions, auto-computes depth; first column of `A` must be positive). Returns a list with entry i equal to the solution vector or `fail` if no (nonneg) integral solution exists. | `[CORE, ZZMOD]` |
