...
| `signature_tuple(L)` | $(p, n)$ | `[INDEF ok]` |
...
| Method | Description | Tags |
|--------|-------------|------|
| `automorphism_group_generators(L)` | Generators of Aut(L) | `[PD, GAP]` |
| `automorphism_group_order(L)` | Order of Aut(L) | `[PD]` |
...
| `local_modification(L, M, p)` | Modify lattice locally at prime $p$ | |
...
| `embed_in_unimodular(L, ...)` | Embed into a unimodular lattice | |
...
- Oscar/Hecke documentation — lattices, genus, classification
